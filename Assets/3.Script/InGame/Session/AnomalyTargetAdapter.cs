...
public class AnomalyTargetAdapter : MonoBehaviour, IAnomalyBody
{
    [Serializable]
    ...
    public bool Supports(string action, MovementSpec movement, out string error)
    {
        ...
        if (!gameObject.activeInHierarchy) { error = "대상 루트는 활성 상태여야 합니다."; return false; }
        bool initiallyVisible = baseline != null ? baseline[0].active : Target.IsVisible;
        if (action == "ObjectAddition" && initiallyVisible) { error = "추가형 사물의 Visual은 초기 상태에서 꺼져 있어야 합니다."; return false; }
        if (action == "ObjectDeletion" && !initiallyVisible) { error = "소실형 사물의 Visual은 초기 상태에서 켜져 있어야 합니다."; return false; }
        ...
        { error = "Visual 바깥 자식에 Trigger 감지 영역을 연결하세요."; return false; }
        if (action == "ShadowAddition" && shadowVisual == null) { error = "그림자 표시 사물이 없습니다."; return false; }
        if (action == "ObjectReplacement" && replacementVisual == null) { error = "교체 사물이 없습니다."; return false; }
        ...
        { error = "그림자 표시는 Visual과 독립된 대상 루트의 자식이어야 합니다."; return false; }
        ...
        { error = "교체 표시는 Visual과 독립된 대상 루트의 자식이어야 합니다."; return false; }
        if (action == "ObjectGroupMovement")
        {
            if (movement.Kind != MovementKind.GroupPosition || groupPoses.Length == 0) { error = "그룹 이동 위치 설정이 없습니다."; return false; }
            foreach (var pose in groupPoses)
                if (pose == null || pose.member == null || !pose.member.IsChildOf(Target.Visual.transform) || pose.member == Target.Visual.transform)
                { error = "그룹 구성원은 Visual 내부 자식이어야 합니다."; return false; }
        }
        if (action == "ObjectMovement" && movement.Kind == MovementKind.GroupPosition) { error = "그룹 이동에는 ObjectGroupMovement가 필요합니다."; return false; }
    ...
    public void Move(MovementSpec movement)
    {
        ...
        if (movement.Kind == MovementKind.Offset) visual.localPosition += axis * movement.Value;
        else visual.localRotation *= Quaternion.AngleAxis(movement.Value, axis);
    }
    ...
    {
        Actionable = enabled;
        interaction ??= GetComponent<Interact>(); interaction.SetInteractionEnabled(enabled);
