...
public class InGameSceneBootstrapper : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        foreach (var item in legacyTestObjects) if (item != null) item.SetActive(false);
    }
    ...
    public void Initialize()
    {
        ...
        try
        {
            ...
            if (areaLayer < 0) throw new InvalidOperationException("InteractionArea 레이어가 없습니다.");
            var data = dataBootstrapper.Data;
            foreach (var pose in cctvPoses) cameras.ConfigureCctvPose(pose.index, mapRoot.TransformPoint(pose.position), mapRoot.TransformPoint(pose.lookAt));
            ...
            {
                if (string.IsNullOrWhiteSpace(row.SceneBinding)) throw new InvalidOperationException($"TargetID={row.ID}: SceneBinding이 없습니다.");
                if (!row.SceneBinding.StartsWith("Interection_Obj/Interaction/", StringComparison.Ordinal)) throw new InvalidOperationException($"TargetID={row.ID}: 허용된 Interaction 경로가 아닙니다.");
            ...
            Host.Initialize();
            if (Host.Session == null || Host.Session.Phase == SessionPhase.ConfigurationError) throw new InvalidOperationException(Host.Session?.Error ?? "회차 초기화 실패");
            Host.enabled = true;
    ...
    {
        if (Application.isPlaying) UnityEngine.SceneManagement.SceneManager.LoadScene(sceneName);
    }
