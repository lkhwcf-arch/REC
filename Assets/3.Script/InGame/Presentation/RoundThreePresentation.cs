...
public sealed class RoundThreePresentation : MonoBehaviour
{
    ...
    private void Handle(SessionEvent message)
    {
        ...
        if (!isActiveAndEnabled || message.RoundId != roundId || message.QuestId != questId)
            return;
        if (message.Kind == "MissionOpened")
            Open();
        if (message.Kind == "MissionResolved" && stage == Stage.Open)
            Resolve();
    }
    private void Open()
    {
    ...
    private void BeginBlackout()
    {
        ...
        for (int i = 0; i < corridorLights.Length; i++)
            if (corridorLights[i] != null)
                lightBaseline[i] = corridorLights[i].intensity;
    ...
    private void Update()
    {
        if (!configured || stage == Stage.Dormant)
            return;
        
        bool pause = Time.timeScale <= 0 || !Application.isFocused;
        standing.SetPaused(pause);
        crawler.SetPaused(pause);
        ...
        if (pause)
            return;
        if (stage is not (Stage.Blinking or Stage.Rushing))
            return;
        ...
        if (stage == Stage.Blinking)
        {
            ...
            if (elapsed >= blinkEnd + Mathf.Max(0, blackoutSeconds))
                BeginRush();
        }
        else
        {
    ...
    private void BeginRush()
    {
        ...
        crawler.Show(rushStart, direction);
        crawler.SetMotion(2);
    ...
    {
        audioSource.Stop(); audioSource.clip = clip;
    ...
    private void SetLights(float multiplier)
    {
        ...
        for (int i = 0; i < corridorLights.Length; i++)
            if (corridorLights[i] != null) 
                corridorLights[i].intensity = lightBaseline[i] * multiplier;
    }
    ...
    {
        if (Application.isPlaying && configured && host.Session.RoundId == roundId)
        {
            ResetPresentation();
            Open();
        }
    ...
    {
        if (Application.isPlaying && configured && stage == Stage.Open)
            Resolve();
    }
#endif
}
